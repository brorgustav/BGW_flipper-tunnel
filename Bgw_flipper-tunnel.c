...
// Change this define if you want both USB and HW serial outputs
#define USE_HW_UART true
#define USE_USB_UART false

int32_t ir_serial_hw_app(void* p) {
    UNUSED(p);
    ...
    if(USE_HW_UART) {
        ...
        furi_hal_uart_transmit(uart_hw, (uint8_t*)"IR‑Serial HW: starting...\r\n", 26, FuriHalWaitForever);
    }

    if(USE_USB_UART) {
        uart_usb = furi_hal_uart_init(FuriHalUartIdUsb, &settings);
        furi_hal_uart_transmit(uart_usb, (uint8_t*)"IR‑Serial USB: starting...\r\n", 27, FuriHalWaitForever);
    }
